...
class DbMigrator extends Command
{
     ...
     * @var string
     */
    protected $description = 'Run db migration classes to import data from the old database';
    ...
    {
        $affected = DbMigratorModel::where('status', MigratorStatus::FAILED->value)
            ->update([
    ...
    protected function interactiveCommand(array $migrations)
    {
           ...
           $selected = $this->choice(
               'Select the db migration to run',
    ...
    {
        $commandBase = 'php artisan mreycode:db-migrate';
        $this->info("Command: {$commandBase} {$actionCommand}");
    }

    protected function getMigrationFiles()
    {
        $migrationDir = app_path('DbMigrators');
        ...
        foreach ($migrationFiles as $file) {
            ...
            $fullyQualifiedClass = "App\\DbMigrators\\{$className}";
    ...
    protected function watchStats($migrator)
    {
        ...
        while (true) {
            ...
            $interval = config('db-migrator.monitoring.interval', self::WATCHER_INTERVAL);
            sleep($interval);
            if($dbData = $migrator->shouldTerminate()) {
                if ($dbData->status === MigratorStatus::FAILED->value) {
                    $message = $dbData->message ?? 'Migration failed with no additional message.';
    ...
    {
        if($lastClass = cache()->get('mreycode-db-migrator:last_run_migration_class')) {
    ...
    {
        cache()->forever('mreycode-db-migrator:last_run_migration_class', $classOption);
    }
    ...
    {
        return config('db-migrator.sequence', []);
    }
}
